    ...
    <section className="py-24 bg-zinc-50 border-y border-zinc-100">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <motion.p
          ...
          className="text-zinc-500 font-light mb-14 max-w-xl"
        >
          ...
          {plans.map((plan, idx) => (
            <motion.div
              ...
              className={`relative flex flex-col gap-6 p-8 rounded-2xl border transition-all ${
                plan.badge
                  ? "bg-blue-600 border-blue-600 shadow-xl shadow-blue-200/60"
                  : "bg-white border-zinc-200 shadow-sm hover:shadow-md"
              }`}
            >
              {plan.badge && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 inline-flex items-center px-3 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-white text-blue-600 shadow-sm border border-blue-100">
                  {plan.badge}
                </span>
              )}

              <div className="flex flex-col gap-2">
                <h3 className={`text-xl font-bold ${plan.badge ? "text-white" : "text-zinc-900"}`}>
                  {plan.tier}
                </h3>
                <p className={`text-sm font-light leading-relaxed ${plan.badge ? "text-blue-100" : "text-zinc-500"}`}>
                  {plan.desc}
                </p>
              </div>

              <div className={`text-center py-4 rounded-xl border ${plan.badge ? "bg-white/10 border-white/20" : "bg-zinc-50 border-zinc-100"}`}>
                <p className={`text-xs font-bold uppercase tracking-wider ${plan.badge ? "text-blue-100" : "text-zinc-400"}`}>
                  Pricing
                </p>
                <p className={`text-xl font-bold mt-1 ${plan.badge ? "text-white" : "text-zinc-900"}`}>
                  Custom Quotation
                </p>
              </div>
              ...
              <ul className="flex flex-col gap-2.5">
                {plan.features.map((feat, i) => (
                  <li key={i} className={`flex items-start gap-2 text-sm ${plan.badge ? "text-blue-100" : "text-zinc-600"}`}>
